...
import {
  ...
  ChevronRight, MoveRight, AlertCircle
} from "lucide-react";
...
export default function TickerDetailPage() {
  ...
  const loadData = useCallback(async (forcedParams?: Record<string, any>) => {
    ...
    try {
      // Latest date is only used to seed the request; the actual EOD date shown
      // comes back from the market endpoint itself (it may resolve to an earlier
      // date if the requested one has no data).
  ...
  const noData = market?.data_available === false;
  const isStale = !noData && [market, vol, risk, liq, fund, agg].some((d: any) => d?.is_stale);

  // EOD date comes from the resolved endpoint, not the latest-date lookup call.
  const displayDate = market?.date ? new Date(market.date).toDateString().toUpperCase() : "—";
      ...
      <div className="max-w-[1600px] mx-auto p-4 sm:p-6 space-y-6">
        ...
        {noData ? (
          <div className="flex flex-col items-center justify-center py-24 text-center border border-slate-800 rounded-2xl bg-slate-900/40">
            ...
            <p className="text-sm font-black text-white uppercase tracking-wide">No Data Available</p>
            <p className="text-[10px] text-slate-500 mt-2 uppercase tracking-widest">
              {ticker} has no recorded history in the system
            </p>
          </div>
        ) : (
          <>
