...
import { 
  ...
  Activity, AlertCircle, Zap, ChevronRight, RefreshCcw, Database
} from "lucide-react";
...
const StaleBadge = () => (
  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg border border-amber-500/30 bg-amber-500/10 shrink-0">
    <AlertCircle className="w-3 h-3 text-amber-400" />
    <span className="text-[8px] sm:text-[9px] font-black text-amber-400 uppercase tracking-widest">Stale Data</span>
  </div>
);
...
export default function PositionPage() {
  ...
  // data_available now comes back on the position-intelligence payload, but there
  // are two distinct "no data" shapes from that endpoint:
  //  1. ticker+manager combo doesn't exist at all -> {"data_available": false}
  //  2. combo exists but has no exposure for the resolved date -> {} (empty object)
  const noData = intel?.data_available === false || !intel || Object.keys(intel).length === 0;
  const isStale = !noData && (summary?.is_stale || intel?.is_stale);

  // EOD date comes from position-summary, which resolves its own date server-side.
  const displayDate = summary?.date ? new Date(summary.date).toDateString().toUpperCase() : "—";
      ...
      {noData ? (
        <div className="max-w-[1600px] mx-auto p-4 sm:p-8">
          <div className="flex flex-col items-center justify-center py-24 text-center border border-slate-800 rounded-2xl bg-slate-900/40">
            ...
            <p className="text-sm font-black text-white uppercase tracking-wide">No Position Data</p>
            <p className="text-[10px] text-slate-500 mt-2 uppercase tracking-widest">
              {manager} has no recorded position in {ticker}
            </p>
          </div>
        </div>
      ) : (
